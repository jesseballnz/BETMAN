...
const tmpDir = path.join(__dirname, 'tmp', 'pulse_live_expiry_after_jump');
const tenantId = 'test_pulse_live_expiry_after_jump';
...
const state = {
  ...
  races: {
    'AUS:Canberra:R4': {
      ...
      runners: [
        { runner_name: 'Scope Watch', fixed_win: 4.0 },
        { runner_name: 'Market Neutral', fixed_win: 6.0 },
      ],
    },
  },
  bet_plans: [],
...
const previousStatus = {
  ...
  interestingRunners: [],
  marketOddsSnapshot: {
    'AUS:Canberra:R4|scope watch': 5.0,
    'AUS:Canberra:R4|market neutral': 6.0,
  },
  marketOddsHistory: {
    'AUS:Canberra:R4|scope watch': [{ ts: Date.now() - 10 * 60 * 1000, odds: 5.0 }],
    'AUS:Canberra:R4|market neutral': [{ ts: Date.now() - 10 * 60 * 1000, odds: 6.0 }],
  },
  marketOddsOpening: {
    'AUS:Canberra:R4|scope watch': 5.0,
    'AUS:Canberra:R4|market neutral': 6.0,
...
assert(!alertsFeed.alerts.some((a) => a.meeting === 'Canberra' && String(a.race) === '4'), 'jumped race should not remain in live Pulse feed');

const alertsHistory = JSON.parse(fs.readFileSync(historyPath, 'utf8'));
const archived = alertsHistory.find((a) => a.meeting === 'Canberra' && String(a.race) === '4' && a.selection === 'Scope Watch');
assert(archived, 'jumped race alert should be preserved in Pulse history');
assert.strictEqual(archived.status, 'settled');
assert.strictEqual(archived.raceSettled, true);

console.log('pulse_live_expiry_after_jump.test.js: ok');
